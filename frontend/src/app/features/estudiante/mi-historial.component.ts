...
@Component({
      ...
      @if (cargando()) {
        ...
        <div class="card resumen">
          ...
          <strong class="resumen__porcentaje">
            {{ h.percentageLast30Days === null ? 'Sin datos todavía' : redondear(h.percentageLast30Days) + '%' }}
          </strong>
        </div>

        <div class="card lista">
          @if (h.attendances.length === 0) {
            <div class="vacio">
  ...
  styles: [`
    ...
    .summary { padding: 1.1rem 1.3rem; display: flex; flex-direction: column; gap: .2rem; }
