...
interface Barra {
  etiqueta: string; total: number; quantity: number;
...
@Component({
      ...
      <section class="card grafico">
        <header class="grafico__cabecera">
          <div>
            ...
            <p class="grafico__sub">Últimos {{ datos().months.length }} meses · por fecha de cobro</p>
          </div>
          <div class="grafico__resumen">
            <span class="resumen__valor">{{ datos().total | number: '1.2-2' }}</span>
            <span class="resumen__etiqueta">total · promedio {{ datos().monthlyAverage | number: '1.0-0' }}/mes</span>
          </div>
        </header>

        <div class="lienzo">
          <svg [attr.viewBox]="'0 0 ' + W + ' ' + H" role="img"
               [attr.aria-label]="'Recaudación de los últimos ' + datos().months.length + ' meses'">
          ...
          @if (activa() !== null && barras()[activa()!]; as b) {
            <div class="globo" [style.left.%]="(b.centro / W) * 100">
              ...
              <span class="globo__pagos">{{ b.quantity }} {{ b.quantity === 1 ? 'pago' : 'pagos' }}</span>
            </div>
          }
        </div>
          ...
          <table>
            ...
            <tbody>
              @for (b of barras(); track b.etiqueta) {
                <tr><th scope="row">{{ b.etiqueta }}</th><td>{{ b.total | number: '1.2-2' }}</td><td>{{ b.quantity }}</td></tr>
              }
            </tbody>
          </table>
        </details>
  ...
  styles: [`
    ...
    .tick, .month { fill: var(--color-text-faint); font-size: 9px; font-variant-numeric: tabular-nums; }
...
export class GraficosIngresosComponent {
  ...
  private readonly techo = computed(() => {
    const max = Math.max(...this.datos().months.map((m) => m.total), 0);
    if (max <= 0) return 100;
  ...
  nombreMesLargo(): string {
    ...
    return ultimo ? `${largos[ultimo.month - 1]} ${ultimo.year}` : '';
  }
}
