...
import org.uteq.backend.deportivo.session.controller.TrainingSessionController;
...
class TrainingSessionControllerTest {
    @Mock private TrainingSessionService sesionService;

    @InjectMocks private TrainingSessionController controller;
    ...
    private SessionTodayResponse response(String entrenador) {
        return new SessionTodayResponse(1L, "SUB-12", entrenador, LocalDate.now(),
    ...
    void hoy_administrador_pide_todas() throws Exception {
        ...
        when(sesionService.todaysSessions("admin@sged.test", true))
                .thenReturn(List.of(response("Carlos"), response("Marta")));
    ...
    void hoy_recepcionista_pide_todas() throws Exception {
        ...
        when(sesionService.todaysSessions("recepcion@sged.test", true))
                .thenReturn(List.of(response("Carlos")));
    ...
    void hoy_entrenador_pide_solo_las_propias() throws Exception {
        ...
        when(sesionService.todaysSessions("carlos@sged.test", false))
                .thenReturn(List.of(response("Carlos")));
    ...
    void mias_delega_username_y_paginacion() throws Exception {
        autenticarComo("carlos@sged.test", "ENTRENADOR");
        when(sesionService.mySessions(eq("carlos@sged.test"), eq(false), anyInt(), anyInt()))
                .thenReturn(List.of(response("Carlos")));
        ...
        verify(sesionService).mySessions("carlos@sged.test", false, 0, 20);
    }
    ...
    void crear_delega_username_autenticado() throws Exception {
        autenticarComo("carlos@sged.test", "ENTRENADOR");
        when(sesionService.create(eq("carlos@sged.test"), any()))
                .thenReturn(response("Carlos"));
