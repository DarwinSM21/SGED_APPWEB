...
import org.uteq.backend.academico.guardian.controller.GuardianController;
...
class GuardianControllerTest {
    ...
    @InjectMocks
    private GuardianController representanteController;
    ...
    void buscarPorId_inexistente_da_404() throws Exception {
        when(representanteService.findById(99L)).thenThrow(new ResourceNotFoundException("no existe"));
    ...
    void desvincularEstudiante_devuelve_204() throws Exception {
        doNothing().when(representanteService).unlinkStudent(anyLong(), anyLong());
