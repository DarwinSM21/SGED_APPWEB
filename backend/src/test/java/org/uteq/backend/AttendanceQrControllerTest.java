...
import org.uteq.backend.deportivo.attendance.controller.AttendanceQrController;
...
class AttendanceQrControllerTest {
    ...
    @Mock private QrAttendanceService qrService;
    ...
    void marcar_token_invalido_da_410() throws Exception {
        when(qrService.redeem("token-malo")).thenReturn(Optional.empty());
    ...
    void marcar_doble_marcado_da_400() throws Exception {
        when(qrService.redeem("token-bueno")).thenReturn(Optional.of(1L));
        when(attendanceService.markByQr(any(), any()))
