...
class ScheduleServiceTest {
    ...
    @Mock private org.uteq.backend.deportivo.attendance.repository.AttendanceRepository attendanceRepository;
    @Mock private org.uteq.backend.deportivo.evaluation.repository.DailyEvaluationRepository evaluacionRepository;
    ...
    void crear_rechaza_horaFin_no_posterior() {
        ...
        var request = new ScheduleRequest(5L, 1, LocalTime.of(17, 0), LocalTime.of(16, 0), null, null);

        assertThrows(IllegalArgumentException.class, () -> service.create("carlos@sged.test", request));
    ...
    void crear_categoria_inexistente_lanza_404() {
        ...
        var request = new ScheduleRequest(999L, 1, LocalTime.of(16, 0), LocalTime.of(17, 0), null, null);

        assertThrows(ResourceNotFoundException.class, () -> service.create("carlos@sged.test", request));
    }
    ...
    void misHorarios_sin_entrenador_asociado_lista_vacia() {
        ...
        assertThat(service.mySchedules("huerfano@sged.test")).isEmpty();
    }
    ...
    void desactivar_ajeno_lanza_404() {
        when(coachRepository.findByUserAccount_Username("carlos@sged.test")).thenReturn(Optional.of(entrenador(1L)));
        when(scheduleRepository.findByIdAndCoachId(50L, 1L)).thenReturn(Optional.empty());

        assertThrows(ResourceNotFoundException.class, () -> service.deactivate("carlos@sged.test", 50L));
        verify(scheduleRepository, never()).save(any());
    }
    ...
    void generarSesionesProgramadas_crea_solo_las_que_faltan() {
        ...
        var horarioYaGenerado = Schedule.builder().idHorario(2L).entrenador(yo).categoria(categoria)
                .diaSemana(diaDeHoy).horaInicio(LocalTime.of(9, 0)).horaFin(LocalTime.of(10, 0)).build();

        when(scheduleRepository.findActiveByDayOfWeek(diaDeHoy))
                .thenReturn(List.of(horarioSinSesionHoy, horarioYaGenerado));
        when(sesionRepository.existsBySchedule_IdAndDate(1L, hoy)).thenReturn(false);
        when(sesionRepository.existsBySchedule_IdAndDate(2L, hoy)).thenReturn(true);

        service.generateScheduledSessions();

        verify(sesionRepository, times(1)).save(any(TrainingSession.class));
    }
    ...
    void generarSesionesProgramadas_cubre_toda_la_semana() {
        ...
        for (short dia = 1; dia <= 5; dia++) {
            ...
            when(scheduleRepository.findActiveByDayOfWeek(dia)).thenReturn(List.of(horario));
        }

        when(scheduleRepository.findActiveByDayOfWeek((short) 6)).thenReturn(List.of());
        when(scheduleRepository.findActiveByDayOfWeek((short) 7)).thenReturn(List.of());
        when(sesionRepository.existsBySchedule_IdAndDate(any(), any())).thenReturn(false);
        ...
        var guardadas = org.mockito.ArgumentCaptor.forClass(TrainingSession.class);
    ...
    private ScheduleRequest peticion(LocalTime inicio, LocalTime fin) {
        return new ScheduleRequest(5L, 1, inicio, fin, "Cancha 2", null);
    }
    ...
    void editar_cambia_la_hora() {
        ...
        when(coachRepository.findByUserAccount_Username("carlos@sged.test")).thenReturn(java.util.Optional.of(yo));
        when(scheduleRepository.findByIdAndCoachId(9L, 1L))
                .thenReturn(java.util.Optional.of(horario));
        when(categoryRepository.findById(5L)).thenReturn(java.util.Optional.of(horario.getCategoria()));
        when(sesionRepository.findBySchedule_IdAndDateGreaterThanEqual(eq(9L), any()))
                .thenReturn(java.util.List.of());

        service.update("carlos@sged.test", 9L, peticion(LocalTime.of(16, 0), LocalTime.of(18, 0)));
    ...
    void editar_rechaza_horaFin_no_posterior() {
        ...
        assertThrows(IllegalArgumentException.class,
                () -> service.update("carlos@sged.test", 9L, peticion(LocalTime.of(18, 0), LocalTime.of(18, 0))));
    }
    ...
    void editar_respeta_la_sesion_con_asistencia() {
        var yo = entrenador(1L);
        var horario = horarioDe(yo);
        var yaUsada = TrainingSession.builder().idSesion(50L).horario(horario)
        ...
        when(attendanceRepository.findBySession_Id(50L)).thenReturn(java.util.List.of(
                org.uteq.backend.deportivo.attendance.entity.Attendance.builder().idAsistencia(1L).build()));
    ...
    void editar_rehace_la_sesion_vacia() {
        var yo = entrenador(1L);
        var horario = horarioDe(yo);
        var vacia = TrainingSession.builder().idSesion(51L).horario(horario)
        ...
        when(attendanceRepository.findBySession_Id(51L)).thenReturn(java.util.List.of());
        when(evaluacionRepository.existsBySession_Id(51L)).thenReturn(false);
    ...
    void crear_rechaza_cruce() {
        ...
        var request = new ScheduleRequest(5L, 2, LocalTime.of(16, 0), LocalTime.of(18, 0), null, null);

        var error = assertThrows(IllegalArgumentException.class,
                () -> service.create("carlos@sged.test", request));
    ...
    void crear_no_valida_la_cancha() {
        ...
        var request = new ScheduleRequest(5L, 2, LocalTime.of(16, 0), LocalTime.of(18, 0),
    ...
    void editar_se_excluye_a_si_mismo() {
        var yo = entrenador(1L);
        Schedule existente = horarioDe(7L, "SUB-16", LocalTime.of(16, 0), LocalTime.of(18, 0));
        when(coachRepository.findByUserAccount_Username("carlos@sged.test")).thenReturn(Optional.of(yo));
        ...
        var request = new ScheduleRequest(5L, 2, LocalTime.of(16, 30), LocalTime.of(18, 30), null, null);
        service.update("carlos@sged.test", 7L, request);

        verify(scheduleRepository).overlapsWith(eq(1L), eq((short) 2), any(), any(), eq(7L));
    }
    ...
    void misHorarios_marca_los_cruces_existentes() {
        ...
        when(scheduleRepository
                .findActiveByCoachOrderByDayAndStartTime(1L))
                .thenReturn(List.of(
