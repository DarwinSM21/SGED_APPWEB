package org.uteq.backend.inventario.movement.controller;
...
public class StockMovementController {
    private final StockMovementService movimientoStockService;
    ...
    public ResponseEntity<Page<StockMovementResponse>> listPaged(Pageable pageable) {
        return ResponseEntity.ok(movimientoStockService.listPaged(pageable));
    }
    ...
    @PreAuthorize("hasAnyRole('ADMINISTRADOR', 'RECEPCIONISTA', 'ENTRENADOR')")
    @Transactional(readOnly = true)
    public ResponseEntity<Page<StockMovementResponse>> listByItem(@PathVariable Long idArticulo, Pageable pageable) {
        return ResponseEntity.ok(movimientoStockService.listByItem(idArticulo, pageable));
    }
    ...
    public ResponseEntity<StockMovementResponse> register(@Valid @RequestBody StockMovementRequest request) {
        var movimiento = movimientoStockService.register(request, authenticatedUsername());
