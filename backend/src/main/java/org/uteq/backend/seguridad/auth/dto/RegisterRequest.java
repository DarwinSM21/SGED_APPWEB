 ...
 *
 * rol es obligatorio: ya no existe un rol generico "USER" al que caer por
 * defecto (no representaba a nadie real de la escuela). Quien llama este
 * endpoint ya es ADMINISTRADOR, asi que puede pedir cualquier rol existente
 * en seguridad.roles. No hay una lista fija aqui a proposito -este proyecto
 * no tiene un enum central de roles, son filas de tabla-; RolRepository.
 * findByNombre es el unico gate, y un nombre que no exista responde 400.
 */
