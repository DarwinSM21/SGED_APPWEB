...
\textbf{Medido de nuevo el 2026-07-30, tras la reestructuración de paquetes
(\texttt{./mvnw test}, 60 clases analizadas): 39,8\,\% de cobertura de
instrucciones (1367 cubiertas de 3432). NO cumple el umbral del
60\,\%.} Las 44 pruebas existentes pasan todas (0 fallos, 0 errores); el
problema no es que algo esté roto, es que los recursos nuevos
(\texttt{Categoria}, \texttt{Entrenador}, \texttt{Usuario}, \texttt{Persona},
\texttt{EstadoGeneral}, más los paquetes vacíos de \texttt{Representante} y
\texttt{Equipo}) no tienen ninguna prueba propia y diluyeron el porcentaje
agregado. Ver sección~\ref{sec:reestructuracion} para el porqué de este
número frente a la cifra de 68,1\,\% que documentaba la versión anterior de
este informe.
...
\emph{Ningún recurso nuevo (Categoria, Entrenador, Usuario, Persona,
EstadoGeneral) tiene fila en esta tabla porque ninguno tiene prueba
dedicada todavía} — es, junto con la encuesta SUS pendiente
(sección~\ref{sec:sus}), el trabajo más urgente antes de tagear
\texttt{v0.9.0-rc}.

Un detalle relevante: en una entrega anterior se descubrió que JaCoCo
...
Ese defecto está corregido; la medición actual sí es real, y por eso puede
mostrar honestamente una regresión.
...
\begin{enumerate}[leftmargin=*]
  \item El sistema cumple con holgura la mayoría de los umbrales
    cuantitativos exigidos: p95 de 14,18\,ms frente a un límite de 200\,ms,
    6 de 6 controles OWASP verificados con evidencia reproducible, y
    accesibilidad Lighthouse de 100/100. \textbf{La cobertura de pruebas no
    cumple} (39,8\,\% medido hoy frente a un mínimo de 60\,\%) — se reporta
    así deliberadamente en vez de repetir la cifra de una medición anterior
    a la reestructuración de paquetes (sección~\ref{sec:cobertura}).
  ...
  \item Quedan abiertos tres frentes, en orden de urgencia: subir la
    cobertura de pruebas de vuelta por encima del 60\,\% antes de tagear
    \texttt{v0.9.0-rc}; la encuesta SUS, que requiere participantes reales
    y no puede fabricarse; y la exposición REST del dominio deportivo
    restante (horarios, sesiones, asistencias, evaluaciones), que es el
    objetivo natural de la Entrega Final.
\end{enumerate}
