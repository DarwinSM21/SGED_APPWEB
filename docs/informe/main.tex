...
vacías, sin tabla en el esquema).
...
\textbf{Medido el 2026-07-30 tras cerrar la brecha de pruebas
(\texttt{./mvnw test}, 60 clases analizadas): 72,5\,\% de cobertura de
instrucciones (2488 cubiertas de 3432). Cumple el umbral del 60\,\%.} El
camino hasta este número quedó documentado en vez de solo reportar el
resultado final: al medir por primera vez después de la reestructuración de
paquetes (sección~\ref{sec:reestructuracion}) el resultado real era
...
de cada uno), y la cobertura subió a 72,5\,\%.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item La distinción entre lo implementado y lo modelado se mantiene
    explícita en todo el documento, incluyendo los casos donde un paquete
    Java existe pero está vacío (\texttt{Representante}, \texttt{Equipo}):
    no se cuentan como funcionalidad entregada solo porque el archivo exista.
