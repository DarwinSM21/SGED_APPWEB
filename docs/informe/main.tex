...
{\ttfamily\url{https://github.com/darcalleg/SGED_APPWEB}}\\[0.6cm]
...
guía. Las cinco corridas independientes de k6 que exige el Bloque~A.1
están archivadas en \ruta{docs/mediciones/perf/} y agregadas con
intervalo de confianza (sección~\ref{sec:discusion}, RQ1).
\textbf{Decisión de alcance, no una tarea sin terminar:} las cinco
corridas miden el mismo escenario con caché caliente. Se evaluó agregar
un escenario de caché fría con contraste inferencial (Wilcoxon con
\emph{Cliff's delta}) frente al caliente, y se descartó: con ambos
escenarios a más de un orden de magnitud del umbral de 200\,ms, el test
confirmaría una diferencia que ninguna de las dos condiciones acerca al
límite (razonamiento completo en la discusión de RQ1,
sección~\ref{sec:discusion}). Por la misma razón no se aplica corrección
por comparaciones múltiples al desglose de usabilidad por perfil
(sección~\ref{sec:sus}): con 3--4 participantes por perfil, una prueba
de hipótesis entre grupos no tiene la potencia para sostenerse, y
corregir un test que no debería correrse en primer lugar sería
estadística de fachada. El dato que gobierna RQ1 —la distancia de p95
al umbral de 200\,ms— ya está medido cinco veces.
...
\texttt{common} & 63,1\,\% (183/290) & 56,0\,\% (56/100) \\
...
\texttt{reportes} & 69,4\,\% (100/144) & 56,8\,\% (25/44) \\
\texttt{seguridad.audit} & 95,7\,\% (90/94) & 82,0\,\% (41/50) \\
...
El subdominio más bajo en líneas es \texttt{common} (63,1\,\%): agrupa
\texttt{common.exception} (manejo de errores, muchas ramas de
\emph{fallback} poco ejercitadas por los tests) y \texttt{common.ia}
...
\begin{table}[htbp]
\centering
\caption{Evidencia cuantitativa de autoría por integrante (\texttt{git log} sobre \texttt{main}, medido el 2026-09-06). La misma medición está en \ruta{CONTRIBUTORS.md}.}
...
\midrule
\textbf{Total en \texttt{main}} & \textbf{310} & \textbf{121\,686} & \textbf{2\,518} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Pallo Pinto Alejandro Daniel} — \emph{Software}, \emph{Análisis
formal}, \emph{Validación}, \emph{Curación de datos}, \emph{Redacción
(borrador original)}, \emph{Visualización}.
...
las 550 actuales —sección~\ref{sec:cobertura}—), la corrección del defecto
...
\emph{Software}, \emph{Investigación}, \emph{Administración del proyecto},
\emph{Supervisión}.
...
\emph{Software}, \emph{Validación}, \emph{Metodología}, \emph{Recursos},
\emph{Redacción (revisión y edición)}.
...
tabla está en \ruta{CONTRIBUTORS.md}.

\begin{table}[htbp]
\centering
\scriptsize
\caption{Cobertura de los catorce roles de la taxonomía CRediT.}
\label{tab:credit-cobertura}
\begin{tabular}{@{}p{3.2cm}p{2.6cm}p{8.0cm}@{}}
\toprule
...
Análisis formal & Alejandro & Análisis estadístico de los datos de rendimiento y usabilidad (intervalos de confianza, distribución \emph{t}). \\
...
Software & Alejandro, Ricardo, Darwin & Implementación de backend (Spring~Boot), \emph{frontend} (Angular) y procedimientos almacenados. \\
...
\url{https://github.com/darcalleg/SGED_APPWEB}, etiqueta \texttt{v1.0.0},
DOI del software en
...
están archivadas con su URL de origen en
\ruta{docs/mediciones/ci/runs-verdes.json}: ocurrieron en
\texttt{DarwinSM21/SGED\_APPWEB}, el repositorio donde de hecho
transcurrió esa ventana de dos semanas, y sus números de corrida
pertenecen al historial de \emph{Actions} de ese repositorio
específico, no del actual.

\textbf{Nota sobre la migración a \texttt{darcalleg/SGED\_APPWEB}
(2026-09-06/07).} El repositorio principal cambió después de que
ocurrió lo anterior; migrar el historial de \texttt{git} no traslada ni
recrea retroactivamente las corridas de \emph{Actions} de otro
repositorio. \texttt{darcalleg/SGED\_APPWEB} tiene su propio historial
de CI, mucho más corto por ser nuevo: 16 corridas verificadas contra su
API pública (\ruta{docs/mediciones/ci/runs-darcalleg.json}), 10 en
...
más reciente al momento de escribir esto (\#18,
\texttt{a46f681}) está en verde.
