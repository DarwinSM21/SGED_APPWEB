...
\textbf{Etiqueta:} & \texttt{v1.0.0} \\[0.2cm]
\textbf{\emph{Commit} del corte:} & el \emph{commit} al que apunta la etiqueta \texttt{v1.0.0} \\[0.2cm]
...
etiqueta \texttt{v1.0.0}, cuyo \emph{commit} consta arriba. Los dos campos
que siguen marcados \pendiente{} —ORCID de cada integrante y DOI del
\emph{dataset}— dependen de registros externos (\url{orcid.org} y el
depósito en Zenodo) y no de este repositorio; se declaran como faltantes
en vez de omitirlos, siguiendo la misma disciplina que \ruta{VERSIONING.md}
ya aplica: no se declara cerrado lo que todavía no lo está.
\end{quote}
...
gestionaba manualmente la matrícula de estudiantes menores de edad,
la asistencia, los pagos y el inventario deportivo, sin trazabilidad
verificable ni control de acceso a datos sensibles. \textbf{Objetivo.}
Este trabajo desarrolla y evalúa SGED, un sistema web que cubre esos
cuatro dominios bajo una estrategia híbrida de acceso a datos (ORM y
procedimientos almacenados parametrizados) y un proceso de ingeniería
de requisitos conforme a ISO/IEC/IEEE~29148:2018. \textbf{Métodos.} Se
siguió \emph{Design Science Research} (Peffers~\emph{et al.}) sobre una
arquitectura C4 (Spring~Boot, Angular, PostgreSQL, Redis, Docker
Compose), con autenticación JWT en cookie \texttt{HttpOnly}. La
evaluación combina pruebas de carga (k6), auditoría de los seis
controles OWASP~Top~10, cobertura de pruebas (JaCoCo), calidad web
(Lighthouse) y usabilidad (\emph{System Usability Scale}, $N=15$), con
datos crudos reproducibles. \textbf{Resultados principales.} El sistema
cumple los umbrales de rendimiento (p95 bajo 200\,ms), los seis
controles OWASP y la cobertura exigida; el proceso de medición
detectó y corrigió tres instrumentos con lecturas engañosas, incluida
una exposición real de datos de menores de edad (hallazgo H-08,
corregido). La usabilidad agregada cruza apenas el umbral aceptable,
pero esconde un patrón bimodal por rol de usuario.
\textbf{Conclusiones.} La estrategia híbrida de acceso a datos es
sostenible sin sacrificar seguridad, y reportar instrumentos
defectuosos —no solo sus resultados— es un aporte metodológico. Queda
abierto el patrón bimodal de usabilidad y la exposición pública del
dominio deportivo restante.
...
itself uncovered and fixed three instruments producing misleading
readings, including a real exposure of underage students' personal
data (finding H-08, now fixed). Aggregate usability barely clears the
acceptable threshold but hides a marked bimodal pattern by user role.
\textbf{Conclusions.} The hybrid data-access strategy is sustainable
without sacrificing security, and reporting faulty measurement
instruments — not just their results — is itself a methodological
contribution. Open work remains on the bimodal usability pattern and
...
por la Entrega Final (sección~\ref{sec:declaraciones}).
...
100\,\% de \emph{branches}. El estado agregado de esa corrida
—86,2\,\% de líneas, 71,2\,\% de \emph{branches}— fue superado después
...
\textbf{61,16\,\% de \emph{branches}}, por debajo del 70\,\% exigido.
Consultada la API de GitHub Actions (Anexo~D), \texttt{main} llevaba
...
\emph{commit}~\texttt{cc891b1} (2026-08-18, \#112) hasta
\texttt{dfa5802} (2026-09-02, \#174), dos semanas completas. La
...
de reactivación y unidad de medida en blanco de \texttt{Articulo} y
\texttt{Representante}. Resultado verificado con \texttt{mvn verify}
completo, en limpio: \textbf{514 pruebas, 0 fallos}; agregada,
\textbf{82,87\,\% de líneas} y \textbf{70,06\,\% de \emph{branches}}
— la regla vuelve a cumplirse, pero con un margen de apenas 0,06 puntos
sobre el umbral: sigue siendo frágil frente al próximo módulo que se
agregue sin pruebas propias, no un problema cerrado de forma permanente.
\end{quote}
...
\textbf{Hallazgo} (Tabla~\ref{tab:moscow})\textbf{:} seis requisitos del módulo deportivo (RF-17 a RF-22,
...
Dos detalles del diagrama de la Figura~\ref{fig:c4-l3} merecen lectura
explícita. Primero,
...
estrechó un 30\,\%, lo que sugiere que la ampliación precisó la
estimación sin desplazarla.
...
priori (sección~\ref{sec:analisis-estadistico})—: media $7{,}02 \pm 0{,}10$\,ms
...
severidad}. No siempre fue así, y vale contarlo: una corrida anterior
(2026-08-14) sí encontró una alerta de severidad \textbf{alta}
...
limpia (\texttt{./mvnw clean test}): 72,5\,\% de cobertura de
instrucciones (2507 cubiertas de 3457), con 102 pruebas y ningún fallo.}
...
\textbf{Estado actual (2026-09-02).} El sistema siguió creciendo después
de la corrida anterior —el módulo \texttt{alerta} completo, un segundo
proveedor de generación de texto, y la maduración de partidos,
alineación, evaluación, sesión y asistencia— y esta vez ese crecimiento
sí hizo caer la regla de cobertura por debajo del umbral (documentado
como hallazgo, no ocultado, en la actualización del 2026-09-02 del
Bloque de Ingeniería de requisitos). Medido con \texttt{./mvnw verify}
—el comando que exige y hace cumplir el umbral, no solo \texttt{test}—
sobre \texttt{jacoco.csv} (\ruta{docs/mediciones/jacoco/}):
\textbf{82,87\,\% de cobertura de líneas} (2583 de 3117) y
\textbf{70,06\,\% de \emph{branches}} (653 de 932), sobre 200 clases
analizadas, con \textbf{514 pruebas} en 67 clases de prueba. Cumple el
\textbf{70\,\%} que exige \texttt{pom.xml} en líneas y en
\emph{branches} —el umbral real del proyecto; una redacción anterior de
este párrafo citaba 60\,\%, que nunca fue el valor configurado— pero
con un margen de apenas 0,06 puntos porcentuales sobre el mínimo de
\emph{branches}: el próximo módulo que se agregue sin pruebas propias
...
\midrule
Servicios & 85,61\,\% (1886/2203) & 71,67\,\% (506/706) \\
Controladores & 81,63\,\% (240/294) & 79,41\,\% (27/34) \\
Otros (config., seguridad, DTO) & 77,62\,\% (444/572) & 64,46\,\% (107/166) \\
Entidades & \textbf{27,08\,\% (13/48)} & \textbf{50,00\,\% (13/26)} \\
\bottomrule
\end{tabular}
\end{table}

Según la Tabla~\ref{tab:cobertura-por-capa}, la capa que queda por
debajo del umbral es la de entidades: la mayoría
de sus líneas son \emph{getters}/\emph{setters} generados por Lombok que
JaCoCo cuenta igual que código de negocio, y las pruebas de servicio y
controlador ejercitan esos métodos de forma indirecta sin que JaCoCo lo
atribuya a la clase de la entidad. No se declara resuelto: es una
limitación real de la medición agregada, no un defecto oculto. Cinco
controladores quedan además en cero por ciento de líneas
—\texttt{ArticuloController}, \texttt{EvaluacionDiariaController},
\texttt{AsignacionController}, \texttt{EspecialidadController},
\texttt{MovimientoStockController}—, todos de módulos añadidos después
de la corrida original y sin prueba propia todavía; queda como trabajo
...
móvil— (Tabla~\ref{tab:lighthouse}):
...
IC 95\,\% & 69,33 $\pm$ 10,46 \quad (58,87 -- 79,79) \\
Mediana & 70,00 \\
Grado & C --- Aceptable \\
\bottomrule
\end{tabular}
...
\begin{itemize}[leftmargin=*]
  \item \textbf{La cobertura de instrucciones no mide calidad de las
    pruebas.} Un 82,87\,\% indica qué proporción del código se ejecuta
...
las 479 actuales —sección~\ref{sec:cobertura}—), la corrección del defecto
...
independientes archivadas en \ruta{docs/mediciones/perf/} (50 usuarios
virtuales, 30\,s por corrida) dan una media de 7{,}02\,ms
(DT~0{,}08; IC~95\,\%~$\pm$~0{,}10) y un p95 promedio de 9{,}91\,ms
(IC~95\,\%~$\pm$~0{,}25) contra el \emph{endpoint} de listado protegido.
El umbral de la norma es 200\,ms con caché caliente: el sistema queda un
orden de magnitud por debajo, y la desviación entre corridas —ocho
centésimas de milisegundo— indica que la medición es estable y no un
resultado afortunado de una ejecución concreta.

La holgura es tan grande que vuelve poco informativo el contraste
estadístico entre caché fría y caliente: con ambos escenarios a más de un
orden de magnitud del umbral, un test de Wilcoxon confirmaría una
diferencia que ninguna de las dos condiciones acerca al límite. Se declara
como decisión, no como omisión: el dato que gobierna RQ1 es la distancia
al umbral, y esa está medida cinco veces.
...
69{,}33 (IC~95\,\%~59{,}77--78{,}89), y la ampliación de diez a quince
...
un 30\,\%. Es decir, los cinco cuestionarios adicionales precisaron la
...
\textbf{no} se repitió: 86{,}12\,\% de líneas (1.924 de 2.234) y
71{,}21\,\% de ramas (413 de 580) sobre el \texttt{main} que incluye ya
el módulo de inventario y el dominio deportivo completo. La comparación
con el episodio de la Segunda Entrega es la que da sentido a la pregunta:
allí la cobertura cayó a 39{,}8\,\% al agregar cinco recursos sin pruebas
propias y hubo que corregirla a 72{,}5\,\%; aquí el crecimiento se
acompañó de pruebas desde el principio y la cifra subió en vez de caer.
...
\begin{enumerate}[leftmargin=*]
  \item El sistema cumple con holgura los umbrales cuantitativos exigidos:
    p95 de 9,91\,ms frente a un límite de 200\,ms, 6 de 6 controles OWASP
    verificados con evidencia reproducible, accesibilidad Lighthouse de
    100/100, y \textbf{82,87\,\%} de cobertura de líneas y
    \textbf{70,06\,\%} de \emph{branches} frente al mínimo de 70\,\%
    (sección~\ref{sec:cobertura}) — cifra que cayó dos veces por debajo
    del umbral a medida que el sistema creció (39,8\,\% tras la
    reestructuración de paquetes, 61,16\,\% tras agregar el módulo de
    alertas y un segundo proveedor de IA) y se recuperó las dos veces con
    pruebas dirigidas al déficit real, no reportando directamente un
    número cómodo ni ocultando que el margen actual sobre el umbral es de
    apenas 0,06 puntos.
...
compuesta sobre una distribución de referencia.
...
el \emph{commit}~\texttt{cc891b1} (\#112, 2026-08-18T04:37Z) hasta
\texttt{dfa5802} (\#174, 2026-09-02T15:56Z) —dos semanas completas—,
...
\midrule
175 & \texttt{a7ccb86} & \emph{test: cierra el gate de cobertura de branches (61\,\% $\to$ 70,06\,\%)} & 2026-09-02 16:32 \\
176 & \texttt{43aa9e9} & \emph{docs(informe): actualizar cifras de cobertura tras cerrar el gate} & 2026-09-02 16:35 \\
177 & \texttt{3ef00eb} & \emph{docs(trazabilidad): RNF-09 citaba el umbral de cobertura equivocado} & 2026-09-02 16:37 \\
\bottomrule
\end{tabular}
\end{table}

Las tres corridas son consecutivas en el historial de \texttt{main} y
verificables en vivo en
\url{https://github.com/DarwinSM21/SGED_APPWEB/actions}.
...
El proceso de diseño/construcción del artefacto está documentado & \cumple & Siete ADR, C4 niveles 1--3 generados desde \ruta{workspace.dsl}, \ruta{docs/observaciones/OBSERVACIONES.md} como bitácora del proceso iterativo \\
...
El artefacto y los datos de evaluación están disponibles para un tercero & \parcial & Repositorio público con DOI de \emph{software}; falta el DOI separado del \emph{dataset} de mediciones (Bloque~D.3, sección~\ref{sec:declaraciones}) \\
...
Evaluación de los 31 requisitos funcionales, 13 no funcionales y 3
...
Checklist completo en \ruta{docs/checklists/incose2023-req.md}.
...
C4 \emph{Complete} & \parcial & Seis RF del módulo deportivo sin campo \texttt{Prioridad:} explícito (sección~\ref{sec:ingenieria-requisitos}) \\
C5 \emph{Singular} & \cumple & Ningún RF combina dos comportamientos verificables por separado; revisión manual línea por línea \\
C6 \emph{Feasible} & \cumple & 28 de 31 RF en estado Implementado son la prueba directa de factibilidad \\
C7 \emph{Verifiable} & \cumple & 31 de 31 RF referencian una prueba automatizada nombrada explícitamente en su entrada del SRS \\
C8 \emph{Correct} & \parcial & RF-11b (peso/altura) es correcto técnicamente pero su base legal no está resuelta (hallazgo H-06) \\
C9 \emph{Conforming} & \cumple & Los 31 RF siguen el mismo patrón \emph{``[condición] [sujeto] deberá [acción] [objeto] [restricción]''} \\
\bottomrule
\end{tabular}
\end{table}
...
C12 \emph{Feasible} & \cumple & 90,3\,\% de los RF implementados por un equipo de tres integrantes en tres entregas previas \\
...
\textbf{Resumen:} 10 de 15 características en \cumple, 5 en \parcial,
0 en \emph{No cumple}. Las cinco parciales comparten una causa común:
el crecimiento del sistema superó la velocidad a la que se
actualizaron \ruta{historias-usuario.md}, \ruta{casos-uso.md} y los
campos de prioridad del SRS —pendiente concreto declarado en la
sección~\ref{sec:ingenieria-requisitos}, no una omisión encontrada
...
Los 50 requisitos individuales de SGED (31 RF, 13 RNF, 3 RD, más tres
recursos de dominio sin RF formal propio) con su historia de usuario,
caso de uso y estado. La versión completa —con endpoint/componente,
...
implementado sin resolución ética (hallazgo H-06,
sección~\ref{sec:ingenieria-requisitos}); RF-19 cubre QR y registro
