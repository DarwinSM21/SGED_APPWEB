...
\textbf{Etiqueta:} & \texttt{v0.9.0-rc} (candidata a versión estable) \\[0.2cm]
\textbf{\emph{Commit} del corte:} & \texttt{380c388} \\[0.2cm]
\textbf{DOI:} & \pendiente{} (Zenodo, ver §\ref{sec:amenazas}) \\
\end{tabular}\\[1.5cm]
...
La Tercera Entrega no añade funcionalidad: verifica, con evidencia empírica
reproducible, lo que el proyecto ya afirmaba tener. Este informe documenta
el estado del sistema SGED en la etiqueta \texttt{v0.9.0-rc}
(\emph{commit} \texttt{380c388}) y los resultados de las cinco mediciones
exigidas.

\textbf{Resultados cuantitativos.} El sistema cumple los cinco umbrales:
rendimiento p95 de 14,18\,ms frente a un límite de 200\,ms (3 corridas k6,
0\,\% de errores); 6 de 6 controles OWASP evidenciados con peticiones
verificables; 72,5\,\% de cobertura de instrucciones frente a un mínimo de
60\,\% (102 pruebas, 0 fallos); accesibilidad Lighthouse de 100/100; y una
media SUS de 68,25 con 10 participantes reales, que cruza el umbral de la
media de la industria por 0,25 puntos.

\textbf{Hallazgo principal.} El aporte real de esta entrega fue descubrir
que \emph{tres de esas mediciones estaban dando resultados válidos en
apariencia sobre un sistema que no hacía lo que el indicador decía}. La
cobertura se medía sobre clases compiladas antes de una reestructuración de
paquetes; la auditoría de control de acceso se bloqueaba a sí misma y
devolvía un rechazo que probaba autenticación, no autorización; y el
registro de usuarios llevaba roto desde la reestructuración mientras sus
pruebas unitarias pasaban, porque usan dobles de prueba y la restricción
violada la aplica el motor de base de datos. Corregidas las tres
mediciones, la auditoría destapó una exposición real: los datos
identificativos de los estudiantes menores de edad —incluida la búsqueda
por número de cédula— eran accesibles a cualquier cuenta autenticada del
sistema, por ausencia de anotaciones de autorización en cinco recursos
nuevos. Está corregido y registrado como hallazgo H-08.

\textbf{Resultado de usabilidad.} La media SUS agregada esconde un patrón
bimodal nítido por perfil: entrenadores y estudiantes califican el sistema
como excelente (86,7 y 90,0; grado A), mientras recepcionistas y padres de
familia lo califican como pobre o inaceptable (51,7 y 43,75; grado F). Se
reporta el patrón y no solo el promedio, porque es el que orienta el
trabajo de la Entrega Final.

\textbf{Lo que no está.} El DOI de Zenodo no está asignado, el módulo de
representantes y el resto del dominio deportivo no están implementados, y
persisten siete hallazgos éticos abiertos. Todos se declaran en las
secciones correspondientes en lugar de omitirse.
...
están versionados sin edición manual. Esta sección declara el procedimiento
antes de presentar los resultados, para que un tercero pueda repetirlos y
obtener cifras comparables.
...
\subsection{Determinismo y trazabilidad}
