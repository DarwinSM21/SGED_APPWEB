...
\textbf{Etiqueta:} & \texttt{v1.0.0} \pendiente{} (ver nota de versión más abajo) \\[0.2cm]
\textbf{\emph{Commit} del corte:} & \pendiente{} (se fija al cerrar la Entrega Final) \\[0.2cm]
\textbf{DOI del software:} & \href{https://doi.org/10.5281/zenodo.21713240}{10.5281/zenodo.21713240} \\[0.2cm]
\textbf{DOI del \emph{dataset}:} & \pendiente{} (depósito separado en Zenodo, Bloque D.3) \\
\end{tabular}\\[1.2cm]

{\large\bfseries Integrantes y ORCID:}\\[0.3cm]
\begin{tabular}{ll}
Arcalle Grefa Darwin Orlando & ORCID: \pendiente{} (registro pendiente en \url{orcid.org}) \\
Pallo Pinto Alejandro Daniel & ORCID: \pendiente{} (registro pendiente en \url{orcid.org}) \\
Velez Lopez Ricardo Elias & ORCID: \pendiente{} (registro pendiente en \url{orcid.org}) \\
\end{tabular}\\[1.2cm]
...
Nota de versión (honesta, no cosmética): al momento de esta redacción el
repositorio sigue en la etiqueta \texttt{v0.9.0-rc}. Los campos marcados
\pendiente{} en esta portada —etiqueta \texttt{v1.0.0}, \emph{commit} de
cierre, ORCID de cada integrante y DOI del \emph{dataset}— se completan
en el mismo commit en que se coloca la etiqueta final, siguiendo la
misma disciplina que \ruta{VERSIONING.md} ya aplica: no se declara
cerrado lo que todavía no lo está.
\end{quote}
...
gestionaba de forma manual y dispersa la matrícula de sus estudiantes
menores de edad, el control de asistencia, los pagos y el inventario
deportivo, sin trazabilidad verificable ni control de acceso a datos
sensibles. \textbf{Objetivo.} Este trabajo desarrolla y evalúa
empíricamente SGED, un sistema web que cubre esos cuatro dominios bajo
una estrategia híbrida de acceso a datos (CRUD sobre ORM más
procedimientos almacenados parametrizados) y un proceso disciplinado de
ingeniería de requisitos conforme a ISO/IEC/IEEE~29148:2018.
\textbf{Métodos.} Se siguió \emph{Design Science Research}
(Peffers~\emph{et al.}) sobre una arquitectura documentada en C4 (Spring
Boot~3.2/Java~21, Angular, PostgreSQL~16, Redis~7, Docker Compose), con
autenticación JWT en cookie \texttt{HttpOnly}. La evaluación empírica
combina pruebas de carga (k6), auditoría de los seis controles
OWASP~Top~10 más análisis estático, cobertura de pruebas (JaCoCo),
calidad web (Lighthouse) y usabilidad percibida (\emph{System Usability
Scale}, $N=10$), todas archivadas como datos crudos reproducibles.
\textbf{Resultados principales.} A la fecha de este corte el sistema
cumple los umbrales de rendimiento (p95 muy por debajo de 200\,ms), los
seis controles OWASP evidenciados y la cobertura mínima exigida; el
proceso de medición detectó y corrigió tres instrumentos que producían
lecturas engañosas, entre ellos una exposición real de datos de
estudiantes menores de edad (hallazgo H-08, corregido). La usabilidad
agregada cruza apenas el umbral aceptable, pero esconde un patrón
bimodal marcado por rol de usuario. \textbf{Conclusiones.} La estrategia
híbrida de acceso a datos es empíricamente sostenible sin sacrificar
seguridad, y la disciplina de reportar instrumentos de medición
defectuosos —en vez de solo sus resultados— es en sí misma un aporte
metodológico del trabajo. Quedan abiertas líneas de trabajo futuro sobre
el patrón bimodal de usabilidad y la exposición pública del dominio
deportivo restante.
...
sports inventory manually and across disconnected spreadsheets, with no
verifiable traceability and no access control over sensitive personal
data. \textbf{Objective.} This work develops and empirically evaluates
SGED, a web system covering those four domains under a hybrid
data-access strategy (parametrized ORM CRUD plus stored procedures) and
a disciplined requirements-engineering process aligned with
ISO/IEC/IEEE~29148:2018. \textbf{Methods.} The project follows Design
Science Research (Peffers~\emph{et al.}) over a C4-documented
architecture (Spring~Boot~3.2/Java~21, Angular, PostgreSQL~16, Redis~7,
Docker Compose), with JWT authentication carried in an
\texttt{HttpOnly} cookie. Empirical evaluation combines load testing
(k6), an audit of the six mandatory OWASP~Top~10 controls plus static
analysis, automated-test coverage (JaCoCo), web-quality auditing
(Lighthouse), and perceived usability (System Usability Scale,
$N=10$), all archived as reproducible raw data. \textbf{Main results.}
As of this cut, the system meets the performance thresholds (p95 well
under 200\,ms), evidences all six OWASP controls, and meets the minimum
required coverage; the measurement process itself uncovered and fixed
three instruments that were producing misleading readings, including a
real exposure of underage students' personal data (finding H-08, now
fixed). Aggregate usability barely clears the acceptable threshold but
hides a marked bimodal pattern by user role. \textbf{Conclusions.} The
hybrid data-access strategy is empirically sustainable without
sacrificing security, and reporting faulty measurement instruments —
not just their results — is itself a methodological contribution of
this work. Open future work concerns the bimodal usability pattern and
the public exposure of the remaining sports domain.
...
especificación: 47 requisitos individuales (31 funcionales, 13 no
...
\begin{enumerate}[leftmargin=*]
  \item Un sistema web de gestión escolar deportiva con arquitectura
    documentada en C4 (niveles 1 a 3) y siete registros de decisión
    arquitectónica, en \ruta{docs/arquitectura/} y \ruta{docs/adr/}.
...
El Capítulo~\ref{sec:marco-teorico} presenta los fundamentos conceptuales
...
capítulos finales —Reproducibilidad, Amenazas a la validez, ética,
CRediT, Discusión, Trabajo futuro y Conclusiones— cierran el documento
con la interpretación crítica y las declaraciones obligatorias exigidas
por la Entrega Final.
...
sobre el código actual —\textbf{270 pruebas, 0 fallos, 140 clases
analizadas}— y el \texttt{jacoco.csv} archivado ya corresponde a esta
corrida, no a la del 30 de julio. Cobertura agregada:
\textbf{75,3\,\% de líneas} (cumple el 70\,\% exigido) pero
\textbf{59,3\,\% de \emph{branches}} (no cumple, aunque mejoró frente al
41,9\,\% de la medición anterior). El déficit no está distribuido
...
también está en 0\,\% (0 de 2). \textbf{Pendiente crítico antes de
colocar la etiqueta \texttt{v1.0.0}:} escribir pruebas para
\texttt{PagoController}/\texttt{PagoService} y \texttt{LesionController}
— es la vía más directa para cerrar la brecha de \emph{branches}, más
que perseguir cobertura dispersa en el resto del código.
...
$\pm$0,25), muy por debajo del umbral de 200\,ms; una corrida de cinco
...
hallazgos. \textbf{Calidad web (Lighthouse) sigue pendiente}: este
entorno de ejecución no tiene un navegador Chrome/Chromium instalable
en esta sesión; queda para una corrida posterior con el entorno
completo.
...
prueba). Cobertura verificada con \texttt{mvn verify} en limpio:
\textbf{369 pruebas, 0 fallos}; agregada, \textbf{86,2\,\% de líneas} y
\textbf{71,2\,\% de \emph{branches}} — ambas superan por primera vez el
70\,\% exigido sobre el \emph{bundle} completo. La regla de JaCoCo pasó
de \texttt{haltOnFailure=false} (advertía en el log sin romper el
\emph{build}) a \texttt{haltOnFailure=true}: el umbral ya se hace
cumplir, no solo se declara. El reporte archivado en
...
forma intermitente según la hora del día en que corriera la suite —sus
...
aplicó sobre el sistema en una etapa comparable a la actual: 10
participantes reales, media SUS 68,25 (apenas por encima del umbral de
68), con un patrón bimodal marcado por perfil que la media agregada
esconde.
...
\node[box] (id) {Registros identificados en las 9 búsquedas\\ $n \approx 85$};
\node[box, below=1.15cm of id] (dedup) {Tras eliminar duplicados y contenido no académico\\ $n \approx 28$};
\node[box, below=1.15cm of dedup] (elig) {Elegibles para verificación bibliográfica completa\\ $n = 12$};
\node[box, below=1.15cm of elig] (inc) {Incluidos en la Tabla~\ref{tab:trabajos-relacionados}\\ $n = 8$};

\node[exc, right=1.6cm of id] (exc1) {Excluidos: blogs, documentación de producto, foros ($n \approx 57$)};
\node[exc, right=1.6cm of dedup] (exc2) {Excluidos: fuera del dominio o del eje técnico del proyecto ($n = 16$)};
\node[exc, right=1.6cm of elig] (exc3) {Excluidos: autoría o venue no verificable contra fuente primaria ($n = 4$)};
...
Los conteos son una reconstrucción honesta del registro de búsqueda del
equipo, no el resultado de una herramienta de gestión bibliográfica
formal (Rayyan, Covidence) — coherente con la reducción de escala que
esta sección declaró al principio.
...
El SRS usa una escala de prioridad de tres niveles (Alta/Media/Baja), no
las cuatro categorías MoSCoW textuales que exige la Entrega Final. La
correspondencia natural es Alta→\emph{Must}, Media→\emph{Should},
Baja→\emph{Could}; la categoría \emph{Won't (esta entrega)} se infiere de
los requisitos explícitamente pausados por una razón declarada, no
simplemente atrasados: RF-11b (peso/altura, pausado por el hallazgo H-06
de \ruta{ETHICS.md}) y el módulo \texttt{equipo}/\texttt{partido}
(esquema versionado en \texttt{V16}, sin API por decisión, no por
omisión).

\begin{table}[htbp]
\centering
\caption{Requisitos funcionales de SGED por categoría MoSCoW inferida.}
\label{tab:moscow}
\begin{tabular}{@{}lrl@{}}
\toprule
\textbf{Categoría MoSCoW (inferida)} & \textbf{Cantidad} & \textbf{Fuente} \\
\midrule
...
\textbf{Sin prioridad formal en el SRS} & \textbf{6} & RF-17,18,19,20,21,22 \\
\midrule
\textbf{Total de requisitos funcionales} & \textbf{31} & RF-01 a RF-30 + RF-11b \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Hallazgo:} seis requisitos del módulo deportivo (RF-17 a RF-22,
los más recientes — implementados entre el 2026-08-09 y el 2026-08-13) no
tienen todavía un campo \texttt{Prioridad:} explícito en el SRS, a
diferencia de los 25 restantes. Es un defecto de forma menor y de
corrección rápida (asignarles Must o Should según su rol en el flujo
operativo), no un defecto de fondo — pero cuenta contra la característica
INCOSE C6 (\emph{Complete}) mientras no se cierre.
...
\textbf{✅ Implementado} en el SRS, pero \ruta{historias-usuario.md}
todavía marca HU-11 y HU-14 como 🟡/⬜, y \ruta{casos-uso.md} sigue
...
\[
\text{tasa de estabilidad} = 1 - \frac{8}{22} \approx 0{,}636 \;\; (63{,}6\,\%)
\]
...
\midrule
Requisitos funcionales totales (RF) & 31 \\
Requisitos no funcionales (RNF) & 13 \\
Restricciones de diseño (RD) & 3 \\
\textbf{Total de requisitos individuales} & \textbf{47} \\
RF en estado ✅ Implementado & 28 (90{,}3\,\%) \\
RF en estado 🟡 Modelado (solo esquema) & 3 (9{,}7\,\%) \\
RF en estado ⬜ Planificado & 0 \\
RF con verificación de prueba nombrada explícitamente & 31 de 31 (100\,\%)$^\ast$ \\
RF con prioridad formal declarada & 25 de 31 (80{,}6\,\%) \\
...
$^\ast$Incluye los 3 requisitos en estado 🟡 Modelado, cuya "verificación"
en el SRS describe la restricción de esquema (p. ej. la restricción
\texttt{CHECK} de \textsc{rf}-19), no todavía una prueba de API porque el
\emph{endpoint} no existe.

Este capítulo satisface las nueve características INCOSE v4 de un
requisito individual de forma desigual: fuerte en \emph{Verifiable} (toda
entrada del SRS nombra su prueba) y en \emph{Correct} (cada corrección se
documenta con su razón, nunca se sobreescribe en silencio); débil en
\emph{Complete} mientras persistan los seis RF sin prioridad formal y las
historias/casos de uso desactualizados señalados arriba. La lista de
verificación INCOSE completa queda pendiente como anexo.
...
dependencias externas (proveedor de correo para notificaciones y, en
producción, el proveedor de TLS). Es el nivel con menor tasa de cambio de
los tres: no se ha modificado desde la Entrega 1A porque los actores y
las fronteras del sistema no cambiaron con la reestructuración en
dominios de la sección~\ref{sec:reestructuracion}, solo su interior.
...
\caption{C4 nivel 3 — componentes del contenedor API Spring Boot, generado desde \ruta{docs/arquitectura/workspace.dsl}.}
\label{fig:c4-l3}
\end{figure}
...
es la fuente de verdad que esta tabla resume:
...
único punto de la capa web:

\begin{lstlisting}[language=Java,caption={Manejador global de excepciones (\ruta{backend/.../common/exception/GlobalExceptionHandler.java})}]
...
\begin{lstlisting}[language=JavaScript,caption={Interceptor de autenticación (\ruta{frontend/src/app/auth/auth.interceptor.ts})}]
...
metodológico, los instrumentos y el procedimiento antes de presentar los
...
rigor y relevancia de Hevner~\emph{et al.}~\cite{hevner2004}, en sus seis
actividades:
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Diseño y desarrollo} — Capítulo~\ref{sec:arquitectura} y
    ...
    híbrida de acceso a datos, siete ADR.
...
correspondiente a RQ1 (sección~\ref{sec:rqs}):
...
omitirlos. \textbf{Limitación declarada:} $N=10$ es una muestra pequeña
y no probabilística; los resultados por perfil (dos participantes por
rol) se interpretan como indicativos de un patrón, no como una
estimación poblacional generalizable —la razón por la que
...
guía. \textbf{Pendiente declarado:} con solo tres corridas de k6
archivadas (sección~\ref{sec:estado-entrega} señala que además están
desactualizadas) no se calculó todavía un test inferencial entre
caché fría y caché caliente ni su tamaño de efecto (por ejemplo Wilcoxon
con \emph{Cliff's delta}); ambos quedan planificados para cuando se
regeneren las cinco corridas independientes que exige el Bloque~A.1.
...
Estado inicial & Base de datos creada desde cero (\texttt{docker compose down -v}) y sembrada con \ruta{db/seed.sql} \\
...
\midrule
C.1 Rendimiento & k6 & \texttt{make bench}: 3 corridas independientes de 50 usuarios virtuales durante 30\,s. Se reportan media, p90, p95 e intervalo de confianza al 95\,\% sobre las tres corridas, no una sola. \\
...
C.3 Usabilidad & SUS en papel & Cuestionario de Brooke de 10 ítems aplicado a 10 participantes externos; transcripción a CSV y cálculo con \ruta{scripts/sus-analysis.py}, cuya aritmética se validó contra los cuatro casos de referencia de la literatura (0, 50, 75 y 100 puntos). \\
...
Cinco corridas independientes de k6 con 50 usuarios virtuales durante 30
segundos contra el \emph{endpoint} protegido \texttt{GET /api/estudiantes}
— cumple el mínimo de cinco corridas que exige el Bloque~A.1 de la
Entrega Final (la Tercera Entrega solo había archivado tres). Corrida
regenerada el 2026-08-14 contra el sistema local completo, tras corregir
el defecto de \ruta{.env} descrito en la sección~\ref{sec:estado-entrega}:

\begin{table}[htbp]
\centering
\caption{Resultados de las corridas de rendimiento (k6).}
\label{tab:k6-resultados}
\begin{tabular}{@{}lrrrrrr@{}}
\toprule
\textbf{Corrida} & \textbf{Media (ms)} & \textbf{Mediana} & \textbf{p90} & \textbf{p95} & \textbf{Errores} & \textbf{RPS} \\
\midrule
...
\midrule
\textbf{Agregado} & \textbf{7,02} & --- & --- & \textbf{9,91} & --- & \textbf{381,12} \\
\bottomrule
\end{tabular}
\end{table}

Con intervalo de confianza al 95\,\%: media $7{,}02 \pm 0{,}10$\,ms
(DT $0{,}08$), p95 $9{,}91 \pm 0{,}25$\,ms, throughput
$381{,}12 \pm 64{,}71$\,RPS. La corrida~4 registró 2 peticiones fallidas
de 15\,921 (0,01\,\%) — el umbral estricto \texttt{rate==0} del
\emph{script} k6 la marca en rojo, y se reporta así en vez de omitirla;
las otras cuatro corridas no tuvieron errores. El throughput de la
corrida~4 también es visiblemente menor (287,88 RPS frente a
$\approx$404 de las demás), coherente con esas dos peticiones fallidas
—probablemente una conexión rechazada bajo el pico de concurrencia—
afectando el cómputo agregado de esa corrida.
...
Dos piezas de evidencia adicionales, generadas el 2026-08-14 y
archivadas en \ruta{docs/mediciones/sec/}, complementan la verificación
manual de los seis controles anteriores.
...
\texttt{/api/docs} (15 URL rastreadas desde la especificación OpenAPI):
\textbf{0 hallazgos de severidad alta}, 7 advertencias de severidad
baja/media, todas concentradas en la interfaz de Swagger UI (una
dependencia de terceros expuesta a propósito para la revisión del
tribunal) —biblioteca JS de terceros potencialmente desactualizada,
cabeceras \texttt{Permissions-Policy}/\texttt{Cross-Origin-Embedder-Policy}
ausentes en esas páginas puntuales, una marca de tiempo Unix dentro de
un archivo \texttt{.js} de terceros— y ninguna sobre los
\emph{endpoints} de negocio de la API. Reporte completo en
\ruta{docs/mediciones/sec/zap/}. \textbf{Alcance declarado:} este
escaneo cubrió el \emph{backend}; una corrida equivalente contra el
\emph{frontend} Angular queda pendiente, junto con la puesta en
producción (Bloque~A.4).

\subsection{Cobertura de pruebas}
\label{sec:cobertura}

\textbf{Medido el 2026-07-30 con construcción limpia
(\texttt{./mvnw clean test}): 72,5\,\% de cobertura de instrucciones
(2507 cubiertas de 3457), con 102 pruebas y ningún fallo. Cumple el umbral
del 60\,\%.} El camino hasta este número quedó documentado en vez de solo
...
resultado real era \textbf{39,8\,\%} — los recursos nuevos
...
el objetivo \texttt{make test} pasó a ejecutar \texttt{clean test}.

\begin{table}[htbp]
\centering
\caption{Clases de prueba JUnit y qué verifica cada una.}
...
\begin{table}[htbp]
\centering
\caption{Estadísticos agregados de la encuesta SUS.}
...
\begin{itemize}[leftmargin=*]
  \item \textbf{La cobertura de instrucciones no mide calidad de las
    pruebas.} Un 72,5\,\% indica qué proporción del código se ejecuta
    durante la suite, no que las aserciones sean pertinentes. El caso del
...
\begin{itemize}[leftmargin=*]
  \item \textbf{Las mediciones de rendimiento se tomaron en la misma
    ...
    14,18\,ms de p95 son un límite inferior optimista: en un despliegue con
    red de por medio la cifra sería mayor, aunque el margen frente al
    umbral de 200\,ms es amplio.
  \item \textbf{Tres corridas son pocas} para caracterizar una
    distribución. Se reporta el intervalo de confianza al 95\,\% para no
    presentar una única corrida como si fuera el valor verdadero, pero con
    $n=3$ ese intervalo es ancho.
...
\begin{itemize}[leftmargin=*]
  \item \textbf{La muestra del SUS es de 10 participantes}, el mínimo
    metodológico, y por conveniencia. El intervalo de confianza
    (54,53--81,97) es lo bastante ancho como para que no pueda afirmarse
    con confianza estadística que la media poblacional supere el umbral de
    68: 68,25 es la mejor estimación puntual disponible, no un aprobado
    consolidado.
  \item \textbf{Los subgrupos por perfil tienen 2 o 3 personas cada uno.}
...
Finalmente, \textbf{el DOI de Zenodo no está asignado} a la etiqueta
\texttt{v0.9.0-rc}. Sin él, el archivo permanente exigido por el Bloque E.2
no está satisfecho y la portada de este informe no puede declararlo. Se
consigna como incumplimiento, no como omisión.
...
\ruta{docs/etica/ETHICS.md} documenta ocho hallazgos, en lugar de afirmar
...
un menor (H-02); no existe mecanismo de supresión de datos —la baja lógica
preserva el historial pero no es un borrado— (H-03); el consentimiento del
representante no está modelado, lo que bloquea el módulo de notificaciones
(H-04); el certificado TLS es autofirmado, adecuado para evaluación pero no
para producción (H-05); peso y altura se agregaron sin base legal
documentada (H-06); y la plantilla de consentimiento cubre a los
evaluadores del SUS, no a los representantes de los menores (H-07).
...
\begin{table}[htbp]
\centering
\caption{Evidencia cuantitativa de autoría por integrante (\texttt{git log}).}
...
\midrule
\textbf{Total en \texttt{main}} & \textbf{121} & \textbf{27\,266} & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Pallo Pinto Alejandro Daniel} — \emph{Software}, \emph{Validación},
\emph{Curación de datos}, \emph{Redacción (borrador original)},
\emph{Visualización}.
...
imágenes por digest. En validación: las 102 pruebas actuales —incluidas
las 57 que cerraron la regresión de cobertura—, la corrección del defecto
...
\emph{Software}, \emph{Administración del proyecto}.
...
sección~\ref{sec:reestructuracion}.

\textbf{Velez Lopez Ricardo Elias} — \emph{Conceptualización},
\emph{Software}, \emph{Metodología}.
...
sobre el que se apoya esta entrega.
...
\textbf{Dos \emph{commits} figuran bajo la cuenta \texttt{DannaN24}}
(\texttt{dninasuntar@uteq.edu.ec}), del 2026-06-29, y son trabajo de
\textbf{Pallo Pinto Alejandro Daniel}: esa era la sesión configurada en el
equipo de la universidad que utilizó ese día. Ya quedaba insinuado en el
propio historial (\emph{commit} \texttt{a456255}, del mismo día). Están
sumados a su fila en la tabla anterior. No se reescribió el historial de
\texttt{git} —hacerlo sobre ramas ya compartidas con el equipo habría
roto sus copias locales—; la aclaración se hace por escrito aquí y en
\ruta{CONTRIBUTORS.md}, que es el mecanismo apropiado para una corrección
de autoría a posteriori.

Adicionalmente, varios \emph{commits} de Ricardo y Darwin usan correos
personales en vez del institucional, y la cuenta de Ricardo aparece con
dos identidades (\texttt{ricardoforever@outlook.es} y
\texttt{ricardo@email.com}). Las cifras de la tabla las consolidan como
una sola persona. Se recomienda unificar \texttt{git config user.email}
...
investigación (sección~\ref{sec:rqs}) con la evidencia empírica disponible
al momento de escribirla. Los bloques marcados \textbf{[ACTUALIZAR]}
dependen de una corrida fresca de \texttt{make bench} / \texttt{make test}
sobre el estado actual de \texttt{main} — la evidencia archivada en
\ruta{docs/mediciones/} antecede al módulo de inventario y a la mitad del
dominio deportivo, así que reportar esos números tal cual sin remedirlos
sería exactamente el error que la sección~\ref{sec:defectos-runtime} ya
documentó una vez (un indicador verde que no refleja el sistema real).}

\paragraph{RQ1 — Rendimiento.} La corrida de referencia disponible midió
p95~=~14{,}18\,ms contra el \emph{endpoint} de listado protegido, muy por
debajo del umbral de 200\,ms con caché caliente. \textbf{[ACTUALIZAR]} esa
cifra corresponde a 3 corridas, no a las 5 independientes que exige la
Entrega Final, y no queda registrado si distingue explícitamente el
escenario de caché fría (umbral 500\,ms) del escenario de caché caliente.
Falta también el test de Wilcoxon con tamaño de efecto (Cliff's delta)
entre ambos escenarios que pide el Bloque~C. Con 5 corridas nuevas por
escenario, RQ1 debería poder responderse con el mismo margen holgado que
ya se observó, pero eso es una expectativa, no todavía un resultado.
...
construido. \textbf{[ACTUALIZAR]} falta el escaneo automático con OWASP
ZAP en modo \emph{baseline} y el análisis estático (find-sec-bugs, regla
\texttt{SQL\_NONCONSTANT\_STRING\_PASSED\_TO\_EXECUTE}) que la Entrega
Final exige como evidencia adicional, reproducible por un tercero sin
depender de esta revisión manual.
...
\textbf{[ACTUALIZAR]} con $N=10$ participantes la desagregación por perfil
es indicativa, no concluyente; subir a $N \geq 15$ (mínimo que exige la
Entrega Final) permitiría un intervalo de confianza más ajustado por
subgrupo.
...
Entrega (sección~\ref{sec:reestructuracion}): la cobertura cayó a 39{,}8\,\%
cuando se agregaron cinco recursos nuevos sin pruebas propias, y se
corrigió a 72{,}5\,\%. La pregunta para la Entrega Final es si ese patrón
se repite con el módulo de inventario y el resto del dominio deportivo,
agregados después de esa medición. \textbf{[ACTUALIZAR]} el reporte
archivado en \ruta{docs/mediciones/jacoco/} es anterior a esos módulos —
no los incluye— y \texttt{pom.xml} hoy exige un mínimo de 60\,\% medido
solo sobre \texttt{LINE}, con \texttt{deportivo/**} y varios paquetes de
\texttt{seguridad}/\texttt{academico} excluidos explícitamente del
chequeo. Antes de poder responder RQ4 con evidencia hace falta una corrida
de \texttt{make test} sobre el \texttt{main} actual, sin exclusiones, con
\texttt{BRANCH} incluido en la regla. La respuesta provisional, mientras
tanto, es una advertencia: el patrón de la Segunda Entrega es exactamente
el tipo de regresión silenciosa que este proyecto ya demostró que puede
ocurrir dos veces si no se vuelve a medir explícitamente.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Mecanismo de supresión de datos.} El sistema aplica baja
    lógica, no borrado físico, por integridad referencial del historial
    deportivo. Si un representante ejerciera el derecho de supresión, hoy
    no hay un procedimiento que lo satisfaga. Se propone un procedimiento
    almacenado de anonimización — sustituir datos identificativos por
    valores neutros conservando claves foráneas y estadísticas agregadas,
    invocable solo por \texttt{ADMINISTRADOR} y registrado en auditoría
    (hallazgo H-03).
  \item \textbf{Base legal documentada para peso y altura.} Estos dos
    campos son datos de salud de un menor sin finalidad de uso
    verificada, en el esquema pero sin ningún \emph{endpoint} de escritura
    confirmado (hallazgo H-06). La línea de trabajo es doble: documentar
    una finalidad concreta con consentimiento separado, o directamente no
    exponerlos por API si ninguna funcionalidad prevista los necesita.
...
\begin{enumerate}[leftmargin=*]
  \item El sistema cumple con holgura los umbrales cuantitativos exigidos:
    ...
    100/100, y \textbf{72,5\,\%} de cobertura de pruebas frente a un mínimo
...
\url{https://github.com/DarwinSM21/SGED_APPWEB}, etiqueta \texttt{v1.0.0}
\pendiente{} (ver nota de versión en la portada), DOI del software en
Zenodo: \href{https://doi.org/10.5281/zenodo.21713240}{10.5281/zenodo.21713240}.
...
repositorio. \textbf{Pendiente:} el depósito separado del \emph{dataset}
de mediciones en Zenodo con DOI propio y licencia Creative Commons
Attribution~4.0, distinto del DOI del software, que exige el Bloque~D.3.

\textbf{Disponibilidad de materiales.} Colección Postman
(\ruta{docs/postman/coleccion.json}, 32 peticiones), \emph{scripts} de
...
P01–P10, y ocho hallazgos éticos (H-01 a H-08, uno ya corregido) están
declarados sin omisiones en \ruta{docs/etica/ETHICS.md} y en la sección
de Consideraciones éticas de este documento.

\textbf{Contribuciones de autores (CRediT).} Los catorce roles de la
taxonomía CRediT asignados a cada integrante, derivados del historial
de \texttt{git} y no autodeclarados, se detallan en la
...
mencionado en este documento (Fly.io, Supabase, Upstash, Zenodo) más
allá del uso de sus capas gratuitas para este proyecto académico.
...
\textbf{Uso de asistentes de inteligencia artificial.} Siguiendo la
