...
| `tab:pruebas-junit` (Cobertura JaCoCo) | `docs/mediciones/jacoco/jacoco.csv`, `jacoco.xml`, `index.html` | `make test` (`clean test`, plugin JaCoCo) (generado) | `73d5114` (regenerado 2026-08-14: 270 pruebas, 75,3 % líneas, 59,3 % branches — branches no cumple el 70 % del Bloque A.1, ver `docs/informe/main.tex` §Estado de la entrega) |
...
| §Escaneo OWASP ZAP baseline (Bloque A.1) | Sistema en ejecución (`docker compose up`, con `SPRINGDOC_ENABLED=false`), sin dato crudo previo | `zap.yaml` (plan de automatización) → `docs/mediciones/sec/zap/zap-report.{html,json,xml}` (generado) | PENDIENTE-DE-COMMIT (regenerado 2026-09-02: la corrida de `73d5114`/2026-08-14 encontró una alerta alta de DOMPurify en Swagger UI; esta repite el escaneo tras apagar esa interfaz en producción — 0 hallazgos, ver §Corrida anterior en `docs/mediciones/sec/zap/REPORT.md`) |
