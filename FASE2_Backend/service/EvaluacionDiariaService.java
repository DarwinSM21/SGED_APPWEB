...
public class EvaluacionDiariaService {
    ...
    /**
     * Crear nueva evaluación diaria (cabecera)
    ...
    public EvaluacionDiariaResponse crearEvaluacion(EvaluacionDiariaRequest request) {
        ...
        // Validar que no exista evaluación para esta sesión
        if (evaluacionRepository.findBySesionIdSesion(sesion.getIdSesion()).isPresent()) {
            throw new IllegalArgumentException("Ya existe una evaluación para esta sesión");
        }
    ...
    public void agregarCalificacion(Long idEvaluacion, Long idEstudiante, DetalleEvaluacionRequest request) {
        ...
        // Validar que la evaluación esté en BORRADOR
        ...
        if (detalle == null) {
            ...
            detalle.setPuntaje(request.getPuntaje());
        }
        
        detalleRepository.save(detalle);
        log.info("Calificación registrada: Eval={}, Est={}, Criterio={}, Puntaje={}",
                idEvaluacion, idEstudiante, request.getIdCriterio(), request.getPuntaje());
    }

    /**
     * Finalizar evaluación (cambiar estado de BORRADOR a FINALIZADA)
     */
    public EvaluacionDiariaResponse finalizarEvaluacion(Long idEvaluacion) {
        ...
        log.info("Evaluación finalizada: {}", idEvaluacion);
        return mapToResponse(evaluacion);
    }
    ...
    /**
     * Obtener promedio de evaluación por estudiante (para dashboard/IA)
     */
    @Transactional(readOnly = true)
